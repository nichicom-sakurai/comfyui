...
**実施結果・既知の制約（実行時に判明）:** 参照動画2本（`動画リンク_IMG_1744.MOV`, `追加動画リンク_IMG_3718.MOV`）を広くサンプリングした結果、両方とも「①基本姿勢」と「②お辞儀→立ち上がり」の反復のみで構成されており、「③座りながら膝伸ばし」の場面は収録されていないことが判明した。`yokkorasho_pose3_kneeext.png`（追加動画リンク_IMG_3718.MOV, 101000ms）は、立ち座り動作中に脚が前方へ踏み出された一過性の瞬間を最も近い近似として採用したものであり、台本が指す「座位のまま下腿を水平に伸ばす」動作とは厳密には一致しない。ユーザー承認済みの既知の制約として扱い、Task8の最終確認・報告でも明記する。
